    ...
    $accion = match ($tipo) {
       ...
       'suma' => function () {
        ...
        /* 'asistencia' => function () {
            ...
            require_once('../controllers/asistenciaControllers.php');
            $model = new asistenciaModel();
            $asistenciaController = new asistenciaControllers($model);
            if (isset($_POST["realizar"])) {
                if ($realizar == 'csv') {
                    $filename = $_FILES["form_csv"]["name"];
                    $info = new SplFileInfo($filename);
                    $extension = pathinfo($info->getFilename(), PATHINFO_EXTENSION);
                    if ($extension == 'csv') {
                        $csv = $_FILES['form_csv']['tmp_name'];

                        $asistenciaController->registrarCsv($csv); 
                        /* echo json_encode('juvenal', JSON_UNESCAPED_UNICODE); 
                    }
                }
            } 
